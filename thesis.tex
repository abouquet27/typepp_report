...
C++ is a general purpose programming language widely-used in a lot of domain
such as operating systems, or machine learning. Types are a core concept in C++
but are source of vulnerabilities. For instance, type confusion is a common
security issue in C++, which occurs when the compiler misinterpret a pointer
type generally after a wrong typecasting and, that can lead to memory corruption
...
(CVE-2023-3079). Hence, it is crucial to detect wrong typecasting which induce
...
\autoref{lst:type_confusion}) and inside a type hierarchy with classes and
object-oriented programming e.g., casting a pointer from a base class to a
derived class.  which can lead to memory corruption and security
vulnerabilities. In this chapter, we will expose the object-oriented
...
solutions to type confusion and how they handle it \autoref{sec:cpp_solutions}.
...
\label{sec:cpp_oop} Object-Oriented Programming (OOP) is about creating objects
...
derived class, and can inherit attributes and methods from other classes which
they they can use or override. In Listing \autoref{lst:type_confusion}, classes
...
derived class to another in type casting by the base class and induce type
...
issues with const\_cast.

\texttt{\textbf{static\_cast}<target-type>(expression)} cast an expression to a
specified type while check at compile-time but not at runtime. Therefore, this
...
underlying bit pattern but with the difference that it does not check neither at
...
check, \texttt{dynamic\_cast} must leverage heavy-weight metadata, Run Time Type
Information (RTTI). However, RTTI is only available when the class has at least
...
existing metadata, such as Runtime Type Information (RTTI) and vtables. 
