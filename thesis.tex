...
Hextype and type++ are two mechanisms that aim to detect type confusion at runtime.
...
We first evaluate each mechanism separately and then compare them. Subsequently,
we will discuss the results and the limitations of the test suite.

\textbf{Type++}

\noindent{}Over the 38 test cases we created, type++ passes 35 and 
fails 3 of them. 2 of the failed due to multiple inheritance. The main issue 
is that type++ does not handle multiple inheritance. The test \texttt{multiple_parent.cpp},
...
The second test case that failed is \texttt{union_casting.cpp}, where we try to cast a pointer on a union
to a pointer of a value inside the union. This test case is not detected as a type confusion by type++.

\textbf{Hextype}

\noindent{}Over the 38 test cases we created, Hextype passes 31 and fails 7 of them.
