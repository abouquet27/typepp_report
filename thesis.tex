...
We built a test suite with a multitude of cases to evaluate how a sanitizer can detect these types confusion and, 
a fortiori, try to prevent such adversaries from exploiting these vulnerabilities,
We particularly focus on the initialization of objects and pointers, because it is at initialization step that 
both methods operate, one by adding a vtable pointer to every object and the other by recording all necessary 
information in its disjoint metadata. Then, we try to create various special test cases to trigger 
type confusion vulnerabilities that can occur in C++ programs. 

We want to compare the results of Hextype and type++, individually and between them. The benchmark quality resides 
...
C++11 without any external libraries or frameworks. 
